...
public:
  ...
  void doFit() {
    ...
    for(int i = 0; i < nparam[3]; i++) {
      fit->SetParameter(i+nparam[0]+nparam[1]+nparam[2], Form("fb_%s",  state_id[i].c_str()), 1., 0.05, 0, 0);
      fit->FixParameter(i+nparam[0]+nparam[1]+nparam[2]);
    }
  ...
  void plotcanv(int nsets, int *sets) {
	...
	f[i_set] = new TF1("cs fit", "[4]*sigplot([0], x, [1], [2], [3], [5], [7], [8]) + (1.-[4])*sigplot([0], x, [1], [2], [3], [6], [7], [8])", ptmmin, 149.9);
    ...
    lc.DrawLatex(xpos, getPos(posif[1]*pow(10,-(nsets-1)), posif[3], 0.1, 1), Form("#beta = %.2f", sigpar[6]));
    ...
    lp.DrawLatex(xpos, getPos(9, -9, 3./20, 0), Form("#beta = %.2f", sigpar[6]));
    ...
    ld.DrawLatex(xpos, getPos(1, -1, 3./20, 0), Form("#beta = %.2f", sigpar[6]));
