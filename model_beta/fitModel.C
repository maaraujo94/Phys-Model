...
public:
  ...
  double myFunction(double *par) {
    ...
    sigpar[6] = par[nparam[0]];
    sigpar[7] = par[nparam[0]+1];
	...
	lumibr *= par[nuis_att[id][i]+nparam[0]+nparam[1]];
      ...
      //for each state, assign sqrt(s)/M, M, beta
      sigpar[0] = datasigma[9][counter]/datasigma[0][counter];
      sigpar[4] = datasigma[0][counter];
      if(datasigma[0][counter] < 5) sigpar[5] = par[nparam[0]+2];
      else sigpar[5] = par[nparam[0]+3];
  ...
  void plotcanv(int nsets, int *sets) {
	...
	sigpar[0] = datasigma[9][counter]/datasigma[0][counter];
	sigpar[4] = datasigma[0][counter];
	if(datasigma[0][counter] < 5) sigpar[5] = fitpar[nparam[0]+2];
	else sigpar[5] = fitpar[nparam[0]+3];
  ...
  void doPlot(const char* state_div) {
    ...
    tex.open("plots/chisquare.tex", std::ios_base::app);
    tex << "\\hline" << endl;
    for(int i = nparam[0]+nparam[1]; i < npartot; i++) {
      tex << "$" << names[i] << "$ &  &";
      if(efitpar[i] != 0) {
