...
//par-> [0]: sqrt(s)/M, [1]: xi, [2]: y, [3]: cosalpha, [4]: M, [5]: beta, [6]: rho, [7]: delta, [8]: k2, [9]: k4
double integf(double *par)
{
  ...
  double x_g = pstar*par[3]/Estar;
  double gfunc = ( pow( Estar+pstar*par[3], -par[6] ) + pow( Estar-pstar*par[3], -par[6] ) ) * ( pow( Estar, par[6] ) / 2.) * pow( 1 + eps - par[3]*par[3], -par[7]) * (1 + par[8]*pow(x_g,2) + par[9]*pow(x_g,4));
...
//par-> [0]: sqrt(s)/M, [1]: xi, [2]: y, [3]: L, [4]: M, [5]: beta, [6]: rho, [7]: delta, [8]: k2, [9]: k4
double sig(vector <double> par)
{
  ...
  double pars[10]={par[0], par[1], par[2], cosa, par[4], par[5], par[6], par[7], par[8], par[9]};
  ...
  double x_g = pNorm*cosaNorm/eNorm;
  double gNorm = ( pow( eNorm+pNorm*cosaNorm, -par[6] ) + pow( eNorm-pNorm*cosaNorm, -par[6] ) ) * ( pow( eNorm, par[6] ) / 2.) * pow( 1 + eps - cosaNorm*cosaNorm, -par[7]) * (1 + par[8]*pow(x_g,2) + par[9]*pow(x_g,4));
...
double sigplot(double sqs_M, double xi, double y, double L, double M, double beta, double rho, double delta, double k2, double k4)
{
  vector <double> pars = {sqs_M, xi, y, L, M, beta, rho, delta, k2, k4};
  return sig(pars);
}

double sig_wrapper(vector <double> par)
{
  vector <double> pars = {par[0], par[1], par[2], par[3], par[4], par[6], par[8], par[9], par[10], par[11]};
  double c1 = sig(pars);
  pars[5] = par[7];
  double c2 = sig(pars);
...
double ptmmin(string det, string state)
{
  if(det == "CDF")
    return 2.0;
  else if(det == "LHCb" && (state == "ups1" || state == "ups2" || state == "ups3"))
    return 2.0;
  else
  return 2.5;
}
