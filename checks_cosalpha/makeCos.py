...
fin = open('../model_final/param_list.txt')
...
for t in range(n_type):
    for rho in rho_v:
        for delta in delta_v:
            ...
            os.system('cp ../model_final/*.C check_%d'%i_c)
            os.system('cp ../model_final/cycle.sh check_%d'%i_c)
            os.system('cp -r ../model_final/plots check_%d'%i_c)
