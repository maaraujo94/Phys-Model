...
void fit()
{
  ...
  aux_int++;
  fit->SetParameter(aux_int, "L_ups1", L_est[1]*100, L_est[1]*10, 0, 0);
  //fit->FixParameter(aux_int);
  aux_int++;
  ...
  fit->FixParameter(aux_int);
  aux_int++;
  fit->SetParameter(aux_int, "beta", 3.0, 0.25, 0, 0);
  //fit->FixParameter(aux_int);
  aux_int++;
  fit->SetParameter(aux_int, "tau", 1, 0.1, 0, 0);
  //fit->FixParameter(aux_int);
  aux_int++;
  fit->SetParameter(aux_int, "rho", 2.3, 0.1, 0, 0);
  //fit->FixParameter(aux_int);
  aux_int++;
  ...
  fit->SetParameter(aux_int, "b", 1.6, 0.1, 0, 0);
  //fit->FixParameter(aux_int);
  aux_int++;
  ...
  //currently at zero while I work on each dataset at once
  aux_int = 0;
