...
int getStyle(string det, int sqs) {
  if(det == "CMS") return 20;
  else if(det == "ATLAS") return 25;
  else if(det == "LHCb") return 26;
  else if(det == "CDF") return 34;  
  else return 1;
}
...
int getCol(double y_avg) {
  double y_min = 0, y_max = 4.5;
  float cv = (n_colors-1)/y_max * y_avg + st_col;
  return do_round(cv);
}


int getCol_s(int i_set) {
